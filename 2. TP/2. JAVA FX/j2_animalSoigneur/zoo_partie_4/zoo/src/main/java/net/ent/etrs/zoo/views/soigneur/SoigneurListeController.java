...
public class SoigneurListeController {
    ...
    @FXML
    public TextField txtRechercheSoigneur;

    private final FacadeSoigneur facadeSoigneur;

    private final ObservableList<Soigneur> soigneurObservableList = FXCollections.observableArrayList();
    ...
    public void initialize() {
        try {
            FilteredList<Soigneur> soigneurFilteredList = new FilteredList<>(soigneurObservableList, s -> true);

            this.txtRechercheSoigneur.textProperty().addListener(obs -> {
                soigneurFilteredList.setPredicate(s -> s.getNom().toLowerCase().startsWith(txtRechercheSoigneur.getText().toLowerCase()) ||
                                                        s.getPrenom().toLowerCase().startsWith(txtRechercheSoigneur.getText().toLowerCase()));
            });

            soigneurObservableList.clear();
            soigneurObservableList.addAll(this.facadeSoigneur.findAll());
            tvSoigneur.setItems(soigneurFilteredList);
