...
public enum Categorie {
    ...
    /**
     * Permet de classer un judoka en connaissant son sexe et son poids.
     * @param pFeminin
     * @param pPoids
     * @return la catégorie correspondante au sexe et poids du judoka
     */
    public static Categorie get(final boolean pFeminin, final int pPoids) {
        for (final Categorie c: values()) {
            if (c.isFeminin() == pFeminin && c.getMinIncluded() <= pPoids && c.getMaxEcluded() > pPoids) {
                return c;
            }
        }
        return null;
