...
public class DemoCollection {
    ...
    public static List<String> retournerNomsPrenomsCycliste(List<Cycliste> lst) {
        ...
        return lst.stream()
                .map(c -> c.getNom().toUpperCase()+" "+c.getPrenom().toUpperCase())
                .toList();
    }
    /**
    ...
    public static List<Cycliste> retournerCyclistesFrequencePlusDe160(List<Cycliste> lst) {
        ...
        return lst.stream()
                .filter(c -> c.calculerFrequenceCardiaqueMax() > 160)
                .toList();
    }
    /**
    ...
    public static void afficherLesDeuxPlusVieuxCycliste(List<Cycliste> lst) {
         lst.stream()
                .sorted((c1,c2)-> -1*Integer.compare(c1.calculerAge(), c2.calculerAge()))
                .limit(2)
                 .forEach(System.out::println);
        //retournerCyclisteParAge(lst).skip(lst.size()-2).forEach(System.out::println);
    }
    /**
    ...
    public static long compterSpecialites(List<Cycliste> lst) {
        // return lst.stream().map(Cycliste::getSpecialite).distinct().count();
        return retournerSpecialites(lst).size();
    }
    /**
    ...
    public static List<Cycliste> retournerLesLeader(List<Cycliste> lst) {
        return lst.stream()
                .filter(s -> s.getSpecialite().equals(TypeSpecialite.LEADER))
                .toList();
    }
